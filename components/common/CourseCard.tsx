...
export default function CourseCard({ course }: CardProps) {
  ...
  {description.split(" ").slice(0, 50).join(" ") + (description.split(" ").length > 50 ? "..." : "")}
          ...
          <Link
            href="/details"
            className="mt-4 ml-auto inline-flex w-35 text-nowrap items-center gap-2 text-sm font-medium text-gray-800 dark:text-gray-200 bg-white/80 px-3 py-1 rounded hover:bg-white transition"
          >
            Course Detail
